...
_EXPECTED_COLUMNS: Dict[str, List[str]] = {
    "gdp": [
        "date_key", "nominal_gdp", "real_gdp", "gdp_growth_yoy",
        "gdp_growth_qoq", "gdp_per_capita",
    ],
    "unemployment": [
    ...
    "credit": [
        "date_key", "total_credit", "credit_nfc", "credit_households",
        "npl_ratio",
    ],
    "interest_rates": [
    ...
    "inflation": [
        "date_key", "hicp", "cpi", "core_inflation",
    ],
    "public_debt": [
        "date_key", "total_debt", "debt_to_gdp_ratio", "budget_deficit",
        "external_debt_share",
    ],
}
...
class DataQualityChecker:
    ...
    def _add(self, name: str, status: str, details: str,
             affected_rows: int = 0, pillar: Optional[str] = None) -> None:
        self.report.checks.append(
            CheckResult(name=name, status=status, details=details,
                        affected_rows=affected_rows, pillar=pillar)
        )
        ...
        for pillar, cols in primary_cols.items():
            ...
            for col in cols:
                ...
                if null_count == len(df):
                    self._add(f"not_null_{pillar}_{col}", "fail",
                              f"Column '{col}' is entirely null",
                              affected_rows=null_count, pillar=pillar)
                elif null_count > 0:
                    self._add(f"not_null_{pillar}_{col}", "warn",
                              f"Column '{col}' has {null_count} nulls",
                              affected_rows=null_count, pillar=pillar)
                else:
                    self._add(f"not_null_{pillar}_{col}", "pass",
                              f"No nulls in '{col}'", pillar=pillar)
        ...
        for pillar, col_ranges in DATA_RANGES.items():
            ...
            for col, (lo, hi) in col_ranges.items():
                ...
                if out_of_range > 0:
                    ...
                    self._add(
                        f"range_{pillar}_{col}", "warn",
                        f"{out_of_range} values outside [{lo}, {hi}] "
                        f"(actual range: [{actual_min:.2f}, {actual_max:.2f}])",
                        affected_rows=int(out_of_range), pillar=pillar,
                    )
                else:
                    self._add(f"range_{pillar}_{col}", "pass",
                              f"All values within [{lo}, {hi}]", pillar=pillar)
    ...
    def check_completeness(self) -> None:
        ...
        for pillar, df in self.data.items():
            ...
            if actual < expected:
                self._add(
                    f"completeness_{pillar}", "warn",
                    f"Expected {expected} rows, got {actual} (missing {expected - actual})",
                    affected_rows=expected - actual, pillar=pillar,
                )
            else:
                self._add(f"completeness_{pillar}", "pass",
                          f"Row count matches expected ({actual})", pillar=pillar)
    ...
    def check_consistency(self) -> None:
        ...
        if credit is not None and {"total_credit", "credit_nfc", "credit_households"}.issubset(credit.columns):
            parts_sum = credit["credit_nfc"].fillna(0) + credit["credit_households"].fillna(0)
            violations = (parts_sum > credit["total_credit"] * 1.01).sum()
            if violations > 0:
                self._add("consistency_credit_components", "warn",
                          f"NFC + Household credit > Total in {violations} rows",
                          affected_rows=int(violations), pillar="credit")
            else:
                self._add("consistency_credit_components", "pass",
                          "Credit components <= total", pillar="credit")

        # Youth unemployment >= general unemployment
        unemp = self.data.get("unemployment")
        if unemp is not None and {"unemployment_rate", "youth_unemployment_rate"}.issubset(unemp.columns):
            mask = unemp["youth_unemployment_rate"].dropna() < unemp["unemployment_rate"]
            violations = mask.sum()
            if violations > 0:
                self._add("consistency_youth_unemp", "warn",
                          f"Youth unemployment < general in {violations} rows",
                          affected_rows=int(violations), pillar="unemployment")
            else:
                self._add("consistency_youth_unemp", "pass",
                          "Youth unemployment >= general", pillar="unemployment")

        # Euribor term structure: 3M <= 6M <= 12M (generally)
        rates = self.data.get("interest_rates")
        if rates is not None and {"euribor_3m", "euribor_6m", "euribor_12m"}.issubset(rates.columns):
            mask = rates["euribor_3m"].dropna() > rates["euribor_12m"].dropna() + 0.1
            violations = mask.sum()
            if violations > 0:
                self._add("consistency_euribor_term", "warn",
                          f"Euribor 3M > 12M + 10bps in {violations} rows",
                          affected_rows=int(violations), pillar="interest_rates")
            else:
                self._add("consistency_euribor_term", "pass",
                          "Euribor term structure consistent", pillar="interest_rates")

        # HICP != CPI (they should differ)
        infl = self.data.get("inflation")
        if infl is not None and {"hicp", "cpi"}.issubset(infl.columns):
            both_valid = infl[["hicp", "cpi"]].dropna()
            if len(both_valid) > 0:
                identical = (both_valid["hicp"] == both_valid["cpi"]).all()
                if identical:
                    self._add("consistency_hicp_cpi", "warn",
                              "HICP and CPI are identical — may indicate missing CPI data",
                              pillar="inflation")
                else:
                    self._add("consistency_hicp_cpi", "pass",
                              "HICP and CPI values differ as expected", pillar="inflation")
        ...
        for pillar, col_ranges in DATA_RANGES.items():
            ...
            for col in col_ranges:
                ...
                if n_outliers > 0:
                    ...
                    self._add(
                        f"outlier_{pillar}_{col}", "warn",
                        f"{n_outliers} value(s) with |Z| > {self._ZSCORE_THRESHOLD} "
                        f"(max |Z| = {worst_z:.1f})",
                        affected_rows=n_outliers, pillar=pillar,
                    )
                else:
                    self._add(
                        f"outlier_{pillar}_{col}", "pass",
        ...
        for pillar, col_ranges in DATA_RANGES.items():
            ...
            for col in col_ranges:
                ...
                if prev is not None:
                    ...
                    if drifts:
                        self._add(
                            f"drift_{pillar}_{col}", "warn",
                            f"Distribution drift detected: {'; '.join(drifts)}",
                            pillar=pillar,
                        )
                    else:
                        self._add(
                            f"drift_{pillar}_{col}", "pass",
    ...
    def check_freshness(self) -> None:
        ...
        for pillar, df in self.data.items():
            ...
            if latest and str(END_YEAR) in str(latest):
                self._add(f"freshness_{pillar}", "pass",
                          f"Latest date_key: {latest}", pillar=pillar)
            else:
                self._add(f"freshness_{pillar}", "warn",
    ...
    def run_all(self) -> DQReport:
        ...
        logger.info(
            ...
            self.report.passed, self.report.warnings,
            self.report.failures, len(self.report.checks),
        )
        return self.report
