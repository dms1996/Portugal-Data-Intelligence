...
from config.settings import RAW_DATA_DIR, START_YEAR, END_YEAR, ensure_directories
...
def _get_with_retry(url: str, params: Optional[dict] = None, headers: Optional[dict] = None,
...
def _fetch_eurostat(dataset: str, key: str,
                    start: str = START_PERIOD,
                    end: str = END_PERIOD) -> pd.DataFrame:
...
def _fetch_eurostat_multi(dataset: str, keys: dict,
                          start: str = START_PERIOD,
                          end: str = END_PERIOD) -> dict:
...
def _fetch_ecb(flow: str, key: str,
               start: str = f"{START_PERIOD}-01",
               end: str = f"{END_PERIOD}-12") -> pd.DataFrame:
    ...
    result = df[["TIME_PERIOD", "OBS_VALUE"]].rename(
...
def _fetch_bpstat(series_ids: list, start: str = f"{START_YEAR}-01-01",
                  end: str = f"{END_YEAR}-12-31") -> dict:
    ...
    for sid in series_ids:
        ...
        url = (f"{BPSTAT_BASE}/domains/{config['domain_id']}"
...
def fetch_gdp() -> pd.DataFrame:
    ...
    series = _fetch_eurostat_multi("namq_10_gdp", {
    ...
    df["date"] = pd.to_datetime(df["period"].str.replace(r"-Q(\d)", r"-\g<1>", regex=True),
                                format="mixed", dayfirst=False)
    # Fix: convert "2023-Q1" -> proper quarter-end date
    ...
    # Round values
    for col in ["nominal_gdp_eur_millions", "real_gdp_eur_millions",
    ...
    result = df[["date", "year", "quarter", "nominal_gdp_eur_millions",
                 "real_gdp_eur_millions", "gdp_growth_rate_yoy",
                 "gdp_growth_rate_qoq", "gdp_per_capita_eur",
                 "source", "country_code"]].copy()
...
def fetch_unemployment() -> pd.DataFrame:
    ...
    series = _fetch_eurostat_multi("une_rt_m", {
    ...
    if not lt_unemp.empty:
        ...
        lt_monthly = lt_monthly.rename(columns={"value": "long_term_unemployment_rate"}).reset_index()
    ...
    if not lfp.empty:
        ...
        lfp_monthly = lfp_monthly.rename(columns={"value": "labour_force_participation_rate"}).reset_index()
    ...
    for col in ["unemployment_rate", "youth_unemployment_rate",
    ...
    result = df[["date", "year", "month", "unemployment_rate",
                 "youth_unemployment_rate", "long_term_unemployment_rate",
                 "labour_force_participation_rate",
                 "source", "country_code"]].sort_values("date").reset_index(drop=True)
...
def fetch_interest_rates() -> pd.DataFrame:
    ...
    if not ecb_rate.empty:
        ...
        ecb_rate = ecb_rate.resample("ME").last().bfill().ffill().reset_index()
    ...
    for col in ["ecb_main_refinancing_rate", "euribor_3m", "euribor_6m",
                "euribor_12m", "portugal_10y_bond_yield"]:
    ...
    cols = ["date", "year", "month", "ecb_main_refinancing_rate",
            "euribor_3m", "euribor_6m", "euribor_12m",
            "portugal_10y_bond_yield", "source", "country_code"]
...
def fetch_inflation() -> pd.DataFrame:
    ...
    rates = _fetch_eurostat_multi("prc_hicp_manr", {
        "hicp":  "M.RCH_A.CP00.PT",
        "core":  "M.RCH_A.TOT_X_NRG_FOOD.PT",
    })

    # Price indices (2015=100)
    indices = _fetch_eurostat_multi("prc_hicp_midx", {
        "energy": "M.I15.NRG.PT",
        "food":   "M.I15.FOOD.PT",
    })
    ...
    result = df[["date", "year", "month", "hicp_annual_rate", "cpi_annual_rate",
...
def fetch_credit() -> pd.DataFrame:
    ...
    for col in ["total_credit_eur_millions", "nfc_credit_eur_millions",
    ...
    cols = ["date", "year", "month", "total_credit_eur_millions",
...
def fetch_public_debt() -> pd.DataFrame:
    ...
    # External debt share not available in Eurostat; leave as NaN
    ...
    df["source"] = "Eurostat (gov_10q_ggdebt, gov_10q_ggnfa)"
    df["country_code"] = "PT"

    result = df[["date", "year", "quarter", "total_debt_eur_millions",
...
def fetch_eu_benchmark() -> pd.DataFrame:
    ...
    countries = ["PT", "DE", "ES", "FR", "IT"]
    country_names = {
        "PT": "Portugal", "DE": "Germany", "ES": "Spain",
        "FR": "France", "IT": "Italy",
    }
...
def _fix_ecb_rate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if n_fixed > 0:
        df.loc[mask, col] = 0.0
        logger.info(
            "  [post-fix] Corrected ECB rate to 0.0%% for %d months "
            "(Oct 2019 – Jun 2022)", n_fixed,
        )
    return df
