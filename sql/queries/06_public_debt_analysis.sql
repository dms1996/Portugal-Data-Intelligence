...
SELECT
    ...
    d.date_key,
    ROUND(pd.total_debt, 2)                           AS total_debt_eur_m,
    ROUND(pd.external_debt_share, 2)                  AS external_debt_share_pct,
    ROUND(100.0 - pd.external_debt_share, 2)          AS domestic_debt_share_pct,
    -- Absolute amounts (estimated from share)
    ROUND(pd.total_debt * pd.external_debt_share / 100.0, 2)
                                                      AS external_debt_eur_m,
    ROUND(pd.total_debt * (100.0 - pd.external_debt_share) / 100.0, 2)
                                                      AS domestic_debt_eur_m,
    -- YoY change in external share
    ROUND(
        pd.external_debt_share
        - LAG(pd.external_debt_share, 4) OVER (ORDER BY d.year, d.quarter), 2
    )                                                 AS external_share_yoy_change_pp
FROM fact_public_debt pd
JOIN dim_date d ON pd.date_key = d.date_key
WHERE pd.external_debt_share IS NOT NULL
ORDER BY d.year, d.quarter;
