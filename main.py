...
def _run_reports() -> StepResult:
    """Generate AI insights and PowerPoint presentation."""
    from src.ai_insights.insight_engine import InsightEngine
    from src.reporting.generate_presentation import generate_presentation

    result = StepResult("Reports")

    # 1. AI / rule-based insights
    ...
    log_section(logger, "STEP 6 / REPORTS: PowerPoint Presentation")
    try:
        pptx_path = generate_presentation()
        result.files.append(str(pptx_path))
        logger.info("PowerPoint saved to: %s", pptx_path)
    except Exception as exc:
        logger.error("PowerPoint generation failed: %s", exc)
        result.errors.append(f"PowerPoint generation failed: {exc}")
...
_MODE_DESCRIPTIONS = {
    ...
    "reports":  "Generate AI insights and PowerPoint presentation",
